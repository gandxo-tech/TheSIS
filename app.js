...
(function () {
  ...
  function initFeed(user) {
    ...
    function loadPosts(reset) {
      ...
      /* FIX/ALGO: pour le fil "Global", on récupère un pool plus large que la
         page affichée (récence Firestore pure), puis on le re-classe
         côté client en mixant récence + engagement + centres d'intérêt,
         et on n'affiche que les PAGE_SIZE meilleurs. Le curseur de pagination
      ...
      var poolSize = isGlobalAlgo ? fd.PAGE_SIZE * 3 : fd.PAGE_SIZE;
      ...
      query.get()
        .then(function (snap) {
          ...
          /* Curseur de pagination : toujours basé sur le pool COMPLET reçu de
             Firestore (avant tri/découpe), pour ne rien sauter ni répéter. */
