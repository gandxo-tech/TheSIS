...
(function () {
  ...
  function initFeed(user) {
    ...
    function loadPosts(reset) {
      ...
      query.get()
        .then(function (snap) {
          ...
          q('feed-empty').style.display = 'none';
      ...
      fd.unsubStories = SIS.db.collection('stories')
        ...
        .onSnapshot(function(snap) {
          ...
          snap.forEach(function(doc) {
            ...
            item.setAttribute('data-story-id', doc.id);
            item.innerHTML =
              '<div class="story-ring' + (d.seenBy && user && d.seenBy[user.uid] ? ' seen' : '') + '">' +
                SIS.renderAvatar({ pseudo: d.authorPseudo||'?', photoUrl: d.authorPhoto||null, size:'sm', gradient: SIS.utils.pseudoToGradient(d.authorPseudo||'') }).replace('av-wrap','av-wrap').replace('</div>','') +
                '<div class="story-av-inner"></div></div>' +
              '<span class="story-label">' + SIS.utils.escHtml(d.authorPseudo||'?') + '</span>';

            /* Build proper story ring */
    ...
    function doPublish() {
      ...
      var postData = {
        ...
        authorPseudo:    identity !== 'anon' ? null : null,
        authorPhoto:     null,
  ...
  function initProfil(user) {
    ...
    function openEditProfil() {
      ...
      if (bioEl) { bioEl.value = p.bio||''; q('edit-bio-count').textContent = (p.bio||'').length; }
