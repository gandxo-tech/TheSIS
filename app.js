...
(function () {
  ...
  function initFeed(user) {
    ...
    function buildQuery() {
      ...
      /* FIX BUG MAJEUR: 'q.where(\'mood\',...)' AJOUTÉ APRÈS un orderBy sur un
         autre champ nécessite un index composite Firestore qui n'existe pas
         sur le projet -> la requête échouait silencieusement
         (FAILED_PRECONDITION) et AUCUN post ne s'affichait dès qu'un filtre
         d'humeur (Chaud/Lourd/Drôle/Dramatique) était sélectionné, sur
         n'importe quel onglet. Le filtrage par humeur se fait maintenant
         côté client dans loadPosts() — voir plus bas. */
      return q;
    }
    ...
    function loadPosts(reset) {
      ...
      /* FIX/ALGO: pour le fil "Global" ET/OU un filtre d'humeur actif, on
         récupère un pool plus large que la page affichée, puis on filtre et/ou
         reclasse côté client (le filtre mood se fait ici, plus dans la
         requête Firestore — voir buildQuery). Le curseur de pagination
         (fd.lastDoc) reste basé sur le DERNIER document du pool complet, pas
         sur le sous-ensemble affiché, pour ne rien sauter/répéter. */
      var isGlobalAlgo = (fd.currentTab === 'global' && !fd.currentMood);
      var hasMoodFilter = !!fd.currentMood;
      var poolSize = fd.PAGE_SIZE;
      if (isGlobalAlgo) poolSize = fd.PAGE_SIZE * 3;
      else if (hasMoodFilter) poolSize = fd.PAGE_SIZE * 5; /* filtrage réduit le rendement, pool plus large */
      ...
      query.get()
        .then(function (snap) {
          ...
          /* FIX: filtrage par humeur côté client (voir buildQuery) */
          var filteredDocs = snap.docs;
          if (hasMoodFilter) {
            filteredDocs = filteredDocs.filter(function (doc) {
              return doc.data().mood === fd.currentMood;
            });
          }

          if (filteredDocs.length === 0 && reset) {
            /* FIX: fd.noMore n'était jamais positionné ici. Comme le feed est
               vide, le sentinel de scroll infini reste visible dès le départ
               (rien à scroller) -> l'IntersectionObserver redéclenchait
               loadPosts(false) en boucle, qui (reset=false) passait dans la
               branche ci-dessous et MASQUAIT le message "aucun post" sans
               jamais le réafficher : l'utilisateur se retrouvait avec un
               écran totalement vide, sans aucun message. */
            fd.noMore = true;
          ...
          /* Curseur de pagination : toujours basé sur le pool COMPLET reçu de
             Firestore (avant filtre/tri/découpe), pour ne rien sauter ni répéter. */
          fd.lastDoc = snap.docs[snap.docs.length - 1];
          if (snap.docs.length < poolSize) fd.noMore = true;

          /* ALGO: pour le fil global, on re-classe le pool par score puis on
             ne garde que les PAGE_SIZE meilleurs de CE lot. */
  ...
  function initChat(user) {
    ...
    function loadSalons() {
      ...
      var query = SIS.db.collection('salons').where('hidden','==',false).orderBy('onlineCount','desc').limit(cd.currentCat ? 100 : 30);
    ...
    function loadCanaux() {
      ...
      /* FIX BUG: '.where(type).where(hidden).orderBy(subscriberCount)' combine
         ...
         sans aucune trace dans la console. Filtre 'type' déplacé côté client. */
      SIS.db.collection('salons').where('hidden','==',false)
        .orderBy('subscriberCount','desc').limit(60).get()
        .then(function(snap) {
          if (!list) return;
          var docs = snap.docs.filter(function(doc){ return doc.data().type === 'canal'; }).slice(0, 20);
          list.innerHTML = '';
          if (docs.length === 0) {
    ...
    function saveProfil() {
      ...
      var doSave = function() {
        ...
        savePromise.then(function() {
          ...
          if (prog) prog.style.display = 'none';
          SIS.toast.error('Erreur sauvegarde');
        });
      };
