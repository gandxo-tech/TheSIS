...
(function () {
  ...
  function initFeed(user) {
    var fd = {
      ...
      PAGE_SIZE:     10,
      userInterests: [],
      followingList: null /* null = pas encore chargée, [] = chargée mais vide */
    };
    ...
    /* ── ABONNEMENTS (pour l'onglet "Abonnements") ── */
    /* FIX: fd.followingList n'était jamais chargée nulle part dans le fichier
       -> l'onglet "Abonnements" retombait TOUJOURS sur le feed global, en
       silence, sans que personne ne s'en rende compte. */
    function loadFollowingList() {
      return SIS.db.collection('follows')
        ...
        .then(function (snap) {
          ...
          snap.forEach(function (doc) {
            ...
            if (t) fd.followingList.push(t);
          });
        })
        .catch(function () { fd.followingList = []; });
    }
    ...
    /* ── ALGORITHME DU FIL — mixte Instagram (récence) / Facebook (engagement
       + pertinence) — appliqué uniquement à l'onglet "Global" (le fil
       principal). Les autres onglets (Viral, CSDPM, Whispers, Battles,
       Abonnements) restent un classement brut/chronologique, comme pour une
       catégorie ou une liste explicite. ── */
    function scorePost(doc) {
      ...
      /* Engagement (façon Facebook) : commentaires/echoes/réactions pèsent plus */
      var reactionsTotal = 0;
      if (d.reactions) {
        for (var k in d.reactions) { if (d.reactions.hasOwnProperty(k)) reactionsTotal += d.reactions[k]; }
      }
      var engagement = (d.commentsCount || 0) * 3 + (d.echoCount || 0) * 4 + reactionsTotal;
      var engagementScore = Math.log(1 + engagement) * 12;

      /* Pertinence : boost si le mood du post correspond à un centre d'intérêt */
      var interestScore = (d.mood && fd.userInterests.indexOf(d.mood) > -1) ? 45 : 0;
    ...
    /* FIX BUG MAJEUR (architecture complète revue) : TOUTE combinaison
       ...
       ces index (qui n'existent pas sur ce projet), CHAQUE requête ci-dessous
       échouait en silence (FAILED_PRECONDITION) :
         - Global (par défaut)      : where(hidden) + orderBy(createdAt)
         - Viral                    : where(hidden) + orderBy(echoCount) + orderBy(createdAt)
         - CSDPM/Whispers/Battles   : where(hidden) + where(type) + orderBy(createdAt)
         - Abonnements              : where(hidden) + where(authorUid in) + orderBy(createdAt)
       Solution : buildQuery() ne pose plus QUE l'orderBy nécessaire au tri
       (un seul champ, jamais combiné à un where sur un autre champ). Tous
       les filtres (hidden, type, abonnements) sont appliqués côté client
       dans loadPosts(), exactement comme pour le filtre d'humeur. Aucun
       index composite n'est donc plus nécessaire, nulle part. */
    function buildQuery() {
      if (fd.currentTab === 'viral') {
        return SIS.db.collection('posts').orderBy('echoCount', 'desc');
      }
      return SIS.db.collection('posts').orderBy('createdAt', 'desc');
    }

    /* Filtre client-side appliqué à chaque doc du pool, en fonction de
       l'onglet et du filtre d'humeur actifs. Remplace tous les .where()
       Firestore retirés de buildQuery() ci-dessus. */
    function clientSideFilter(doc) {
      var d = doc.data();
      if (d.hidden) return false;

      if (fd.currentTab === 'csdpm'   && d.type !== 'csdpm')   return false;
      if (fd.currentTab === 'whispers'&& d.type !== 'whisper') return false;
      if (fd.currentTab === 'battles' && d.type !== 'battle')  return false;
      if (fd.currentTab === 'following') {
        var list = fd.followingList;
        if (!list || list.length === 0) return false; /* pas d'abonnement -> rien dans cet onglet */
        if (list.indexOf(d.authorUid) === -1) return false;
      }
      if (fd.currentMood && d.mood !== fd.currentMood) return false;
    ...
    function loadPosts(reset) {
      ...
      var isGlobalAlgo = (fd.currentTab === 'global' && !fd.currentMood);
      var hasClientFilter = (fd.currentTab !== 'global') || !!fd.currentMood;
      var poolSize = fd.PAGE_SIZE;
      if (isGlobalAlgo) poolSize = fd.PAGE_SIZE * 3;
      else if (hasClientFilter) poolSize = fd.PAGE_SIZE * 5; /* filtrage réduit le rendement, pool plus large */
      ...
      query.get()
        .then(function (snap) {
          ...
          /* FIX: filtrage côté client (hidden + type d'onglet + abonnements
             + humeur) — voir buildQuery() pour le pourquoi. */
          ...
          /* ALGO: pour le fil global, on re-classe le pool par score puis on
             ne garde que les PAGE_SIZE meilleurs de CE lot. Pour les autres
             onglets (déjà filtrés côté client ci-dessus), on découpe juste
             à la taille de page habituelle. */
          var docsToRender = filteredDocs;
          if (isGlobalAlgo && docsToRender.length > 0) {
            ...
            docsToRender = docsToRender.slice(0, fd.PAGE_SIZE);
          }
        ...
        .catch(function (err) {
          ...
          /* FIX MAJEUR: avant, en cas d'erreur d'index Firestore manquant,
             le code réinitialisait juste fd.currentTab/fd.currentMood et
             affichait un toast — SANS JAMAIS relancer le chargement. L'écran
             restait vide indéfiniment (cf. capture : "Chargement simplifié"
             affiché puis... rien). On relance maintenant explicitement avec
             la requête la plus simple possible (fil global, sans filtre). */
          if (err.code === 'failed-precondition' || (err.message && err.message.indexOf('index') > -1)) {
            if (fd._indexFallbackDone) {
              /* Le fallback simple a déjà échoué une fois -> on arrête là
                 pour éviter une boucle infinie de requêtes. */
              SIS.toast && SIS.toast.error('Chargement impossible', 'Réessaie plus tard.');
              return;
            }
            fd._indexFallbackDone = true;
            fd.currentTab = 'global';
            fd.currentMood = '';
            fd.lastDoc = null;
            fd.noMore = false;
            SIS.toast && SIS.toast.info('Chargement simplifié', 'Affichage du fil complet en attendant.');
            loadPosts(true);
          } else {
    ...
    function openPacte(callback) {
      ...
      /* FIX: where(hidden)+orderBy(createdAt) sur champs différents nécessite
         un index composite -> requête souvent en échec silencieux, et sans
         .catch(), le callback n'était jamais rappelé en cas d'erreur. */
      SIS.db.collection('posts')
        ...
        .catch(function(err) {
          console.warn('Pacte load err', err);
          var prev = q('pacte-post-preview');
    ...
    function bindEvents() {
      ...
      qsa('.tab-item', q('feed-tabs')).forEach(function(tab) {
        tab.addEventListener('click', function() {
          ...
          fd._indexFallbackDone = false; /* on retente un chargement normal */
  ...
  function initChat(user) {
    ...
    function renderSalonItem(id, d) {
      ...
      var avHtml = d.photoUrl
        ? '<div class="salon-av"><img src="' + SIS.cloudinary.url(d.photoUrl,'thumb') + '" loading="lazy" alt="Avatar du salon"></div>'
    ...
    function openRoom(roomId, data) {
      ...
      if (roomAvEl) {
        if (data.photoUrl) {
          roomAvEl.innerHTML = '<img src="' + SIS.cloudinary.url(data.photoUrl,'thumb') + '" style="width:100%;height:100%;object-fit:cover" alt="Avatar">';
  ...
  function initProfil(user) {
    ...
    function renderProfile(p) {
      ...
      if (vEl) {
        ...
        /* FIX: where(authorUid)+where(echoCount>=5) sur des champs
           ...
           compte côté client. */
        SIS.db.collection('posts').where('authorUid','==',user.uid).limit(300).get()
          .then(function(snap){
    ...
    function loadViralPosts() {
      ...
      SIS.db.collection('posts').where('authorUid','==',user.uid).limit(200).get()
        .then(function(snap) {
