...
(function (SIS) {
  ...
  SIS.cloudinary = {
    cloudName:    'duddyzckz',
    /* FIX BUG CRITIQUE: 'ml_defaulte' (avec un 'e' final) ne correspond à
       aucun preset Cloudinary standard -> le nom auto-généré par Cloudinary
       pour un preset non signé est 'ml_default'. Si ce preset exact n'existe
       pas dans le compte, TOUS les uploads (avatar, couverture, posts,
       stories, certification) échouent. ⚠️ Vérifie dans ton dashboard
       Cloudinary (Settings > Upload > Upload presets) que le preset s'appelle
       bien 'ml_default' et qu'il est en mode "Unsigned" — si tu as créé un
       preset avec un autre nom, remplace la valeur ci-dessous par celui-ci. */
