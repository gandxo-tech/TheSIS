...
(function (SIS) {
  ...
  SIS.image = (function () {
    ...
    /* FEATURE : upload audio (messages vocaux). Cloudinary n'accepte les
       fichiers audio que sur le endpoint /video/upload (l'audio est traité
       comme une "resource_type: video" côté Cloudinary) — le endpoint
       /image/upload utilisé par upload() rejetterait un blob audio. */
    function uploadAudio(blob, onProgress) {
      return new Promise(function (resolve, reject) {
        var file = blob instanceof File ? blob : new File([blob], 'sis_voice_' + Date.now() + '.webm', { type: blob.type || 'audio/webm' });
        var fd = new FormData();
        fd.append('file', file);
        fd.append('upload_preset', SIS.cloudinary.uploadPreset);
        ...
        xhr.onload = function () {
          ...
          if (xhr.status === 200 && res) {
            resolve({ publicId: res.public_id, url: res.secure_url, duration: res.duration || 0 });
          } else {
            var realMsg = (res && res.error && res.error.message) || xhr.responseText || ('HTTP ' + xhr.status);
            console.error('Upload audio Cloudinary échoué [' + xhr.status + ']:', realMsg);
            reject(new Error(realMsg));
          }
        };
        xhr.onerror = function () { reject(new Error('Erreur réseau upload audio')); };
        xhr.send(fd);
      });
    }
    ...
    return {
      ...
      audioUrl: function (publicId) {
        return publicId ? 'https://res.cloudinary.com/' + SIS.cloudinary.cloudName + '/video/upload/' + publicId : '';
      },
  ...
  SIS.authHelper = (function () {
    ...
    /* FEATURE: observer la présence RÉELLE d'un autre utilisateur (RTDB),
       en direct. Remplace les indicateurs "En ligne" statiques/en dur qui
       ne reflétaient jamais l'état réel de l'autre personne. Retourne une
       fonction de désabonnement, à appeler quand la vue se ferme. */
    function watchPresence(uid, callback) {
      if (!uid || !SIS.rtdb || typeof callback !== 'function') {
      ...
      var handler = function (snap) {
        var v = snap.val() || {};
        callback({ online: !!v.online, lastSeen: v.lastSeen || null });
      };
  ...
  SIS.profilePopup = (function () {
    ...
    function _render(pseudo, opts) {
      ...
      var nameEl = overlay.querySelector('.pp-name');
      if (nameEl && opts.uid) {
